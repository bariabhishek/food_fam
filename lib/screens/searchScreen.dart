                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, int index) {
                            return suggested(
                                index,
                                context,
                                aprovedlist[index].name,
                                aprovedlist[index].mobile,
                                aprovedlist[index].status,
                                aprovedlist[index].orderDetails,
                                aprovedlist[index].id);
                          },
                        ),
                      )
              ],
        ...
        if (data['success'] == 1) {
          ...
          for (int i = 0; i < dlist.length; i++) {
            ...
            if (status.toString() == "A") {
              print('getting data' + name.toString());
              aprovedlist.add(new AproverdOrders(id, name, mobile, status,
                  reason, date, instruction, orderDetails));
            } else if (status.toString() == "P") {
              pendinglist.add(new PendingOrders_(id, name, mobile, status,
                  reason, date, instruction, orderDetails));
            }
...
/*
  ...
  Widget UiSearch(int index, BuildContext context, String name, String mobile,
      String status, List order, String id) {
    print('working');
    if (status == 'P') {
      return Card(
        child: Container(
          ...
          child: ListTile(
            onTap: () {
              AppRoutes.goto(
                  context,
                  OrderDetailsPic("pending", id, name, mobile, status,
                      pendinglist[index].orderDetails));
            },
            title: Text(name),
  ...
  Widget suggested(int index, BuildContext context, String name, String mobile,
      String status, List order, String id) {
    ...
    if (status == 'A') {
      return InkWell(
        onTap: () {
          AppRoutes.goto(
              context,
              OrderDetailsPic("approverd", id, name, mobile, status,
                  aprovedlist[index].orderDetails));
        },
        child: Card(
          child: Container(
